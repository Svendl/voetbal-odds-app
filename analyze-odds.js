// api/analyze-odds.js
// Dit is je beveiligde backend - API key zit VEILIG hier

import Anthropic from "@anthropic-ai/sdk";

export default async (req, res) => {
  // Alleen POST requests allowed
  if (req.method !== "POST") {
    return res.status(405).json({ error: "Method not allowed" });
  }

  try {
    // Haal API key op (uit Vercel environment)
    const apiKey = process.env.CLAUDE_API_KEY;
    if (!apiKey) {
      return res.status(500).json({ error: "API key niet geconfigureerd" });
    }

    // Haal data van frontend op
    const { analysisType, league, date } = req.body;

    // Bouw prompt op basis van type analyse
    let userPrompt;
